...
현재 후보 firmware ID와 ROS 기대값은 **0x00024903**으로 함께 갱신했다.
...
좌표 변환 → MoveIt**. RGB-D 주행 지도 작성은 현재 경로에 포함하지 않는다.
...
리프트 하중 유지 증거가 아니다.** 기존 `system_stop`의 전체 hold 연결은 별도다.
...
모드 WRITE·장치 모델 discovery·boot ID·리프트 원점/높이/hold와 전체 로봇 hold 연결은
남아 있다. 이번 fake HAL 통합은 실제 reader/writer/gate/core를 실행하며 RX ring과
